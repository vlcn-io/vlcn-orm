// SIGNED-SOURCE: <3233cc3d3016b73438be72232bf05fd8>
...
export default class DeckQuery extends DerivedQuery<Deck> {
  static create() {
    return new DeckQuery(QueryFactory.createSourceQueryFor(spec), modelLoad(spec.createFrom));
  }
  ...
  whereId(p: Predicate<Data['id']>) {
    return new DeckQuery(this, filter(new ModelFieldGetter<'id', Data, Deck>('id'), p));
  }

  whereName(p: Predicate<Data['name']>) {
    return new DeckQuery(this, filter(new ModelFieldGetter<'name', Data, Deck>('name'), p));
  }

  whereCreated(p: Predicate<Data['created']>) {
    return new DeckQuery(this, filter(new ModelFieldGetter<'created', Data, Deck>('created'), p));
  }

  whereModified(p: Predicate<Data['modified']>) {
    return new DeckQuery(this, filter(new ModelFieldGetter<'modified', Data, Deck>('modified'), p));
  }

  whereOwnerId(p: Predicate<Data['ownerId']>) {
    return new DeckQuery(this, filter(new ModelFieldGetter<'ownerId', Data, Deck>('ownerId'), p));
  }

  whereSelectedSlide(p: Predicate<Data['selectedSlide']>) {
    return new DeckQuery(
      this,
      filter(new ModelFieldGetter<'selectedSlide', Data, Deck>('selectedSlide'), p),
    );
  }
