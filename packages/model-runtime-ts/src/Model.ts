...
export default class Model<T extends Object, E extends Object = {}> implements IModel<T> {
  private subscriptions: Set<() => void> = new Set();
  private keyedSubscriptions: Map<keyof T, Set<() => void>> = new Map();

  private triggers: Map<keyof E, Set<(any) => (Changeset<any, any> | null)[]>> = new Map();
  ...
  addTrigger<K extends keyof E>(event: K, cb: (data: E[K]) => (Changeset<any, any> | null)[]) {
  ...
  protected trigger<K extends keyof E>(event: K, data: E[K]): Changeset<any, any>[] {
  ...
  isNoop(newData: Partial<T>): boolean {
    return Object.entries(newData).every(entry => this._data[entry[0]] === entry[1]);
  }
