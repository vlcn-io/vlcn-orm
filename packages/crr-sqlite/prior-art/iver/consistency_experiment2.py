...
class TestNode():
    def __init__(self, databases, id, db_name, uuids, mode, mergeInterval):
    ...
    def mergeRandom(self):
        ...
        mdb = self.databases[random.randint(0, len(self.databases)-1)]
        while mdb == self.name:
            mdb = self.databases[random.randint(0, len(self.databases)-1)]

        self.c.execute("ATTACH DATABASE '"+mdb+"' as 'db2';")
        self.conn.commit()
        sqlFromFile(self.c, "crr_merge.sql")
    ...
    def doChaosUpdate(self):
        ...
        if self.mergeInterval != 0:
            if self.nUpdates%self.mergeInterval == 0:
                self.mergeRandom()
        self.nUpdates += 1
...
def main(initialRows, numNodes, mergeInterval):
    ...
    db_nodes = []
    for i  in range(1, numNodes+1):
        db_nodes.append(TestNode(db_names, i, db_names[i-1], uuids, True, mergeInterval))

    c_node = TestNode(db_names, 0, "test_dbs/controll_db", uuids, False, mergeInterval)
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.savefig('plots/consistency2_'+ lbl +'.png', bbox_inches='tight')
    plt.show()
