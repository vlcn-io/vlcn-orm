...
const identifier = () => fc.array(idChar()).map(arr => arr.join(''));
...
test('SELECT * FROM T', () => {
  fc.assert(
    fc.property(identifier(), table => {
      expect(sql`SELECT * FROM ${'T'}`(table).toString('sqlite')).toEqual([
        'SELECT * FROM `' + table + '`;',
        [],
      ]);
    }),
...
test('SELECT C FROM T', () => {
  fc.assert(
    fc.property(identifier(), identifier(), (column, table) => {
      expect(sql`SELECT ${'C'} FROM ${'T'}`(column, table).toString('sqlite')).toEqual([
