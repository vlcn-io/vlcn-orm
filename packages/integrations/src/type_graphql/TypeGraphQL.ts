...
import { nodeFn, fieldFn, tsImport } from '@aphro/schema';
import { Node, Field } from '@aphro/schema-api';
...
export class TypeGraphQL implements AphroditeIntegration {
  ...
  private getGraphQLType(field: Field): string {
    switch (field.type) {
      ...
      default:
        throw new Error(`${field.type} is not yet support by the TypeGraphQL integration`);
    }
  }
}
